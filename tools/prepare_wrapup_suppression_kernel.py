"""Build a self-contained Kaggle CPU kernel for the wrap-up-suppression probe.

Mirrors `tools/prepare_hops_calibration_kernel.py` but targets
`notebooks/wrapup_suppression.ipynb`. CPU machine (no GPU, so the run does not
consume the weekly GPU quota). The bootstrap cell recreates the repo files the
probe needs; the SDK and model servers come from the attached competition
dataset. `probe_wrapup_suppression.py` imports `probe_gptoss_compression` for its
candidate builder, so that file is embedded too.
"""
...
DEFAULT_OUT_DIR = REPO / "kaggle_push" / "wrapup_suppression"
DEFAULT_KERNEL_ID = "canqiang/aiagsec-wrapup-suppression"
DEFAULT_TITLE = "AIAgSec Wrapup Suppression"
NOTEBOOK_NAME = "wrapup_suppression.ipynb"
EMBEDDED_FILES = (
    "tools/probe_wrapup_suppression.py",
    "tools/probe_gptoss_compression.py",
