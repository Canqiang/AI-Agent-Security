"""Build a self-contained Kaggle kernel folder for GGUF validation.
...
`notebooks/validation.ipynb` into a generated notebook under `kaggle_push/`,
so the remote kernel can recreate `src/` and `tools/` before running.
"""
...
DEFAULT_OUT_DIR = REPO / "kaggle_push" / "validation"
DEFAULT_KERNEL_ID = "canqiang/aiagsec-gguf-validation"
DEFAULT_TITLE = "AIAgSec GGUF Validation"
EMBEDDED_FILES = (
    "src/attack.py",
...
def bootstrap_cell() -> dict[str, Any]:
    ...
    source = (
        ...
        "print('bootstrapped validation files:', ', '.join(sorted(FILES)))\n"
    )
    return {
        "cell_type": "code",
...
def build_kernel(out_dir: Path, *, kernel_id: str, title: str) -> dict[str, Any]:
    notebook = load_notebook(REPO / "notebooks" / "validation.ipynb")
    ...
    cells.insert(
        1,
        {
            ...
            "source": [
                "## Bootstrap\n",
                "\n",
                "This generated cell recreates the repo files required by the validation runner."
            ],
        },
    )
    ...
    notebook_path = out_dir / "validation.ipynb"
